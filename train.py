...
from dataset import DAVIS_Train_DS, PreTrain_DS, YouTube_Train_DS
...
def get_args():
    ...
    parser.add_argument(
        "--dataset", type=str, default=None, required=True, help="Dataset folder."
    )
    parser.add_argument("--seed", type=int, default=-1, help="Random seed.")
    parser.add_argument("--log", action="store_true", help="Save the training results.")
    parser.add_argument(
        "--level", type=int, default=0, help="0: pretrain. 1: DAVIS. 2: Youtube-VOS."
    )
    parser.add_argument(
        "--lr", type=float, default=1e-5, help="Learning rate, default 1e-5."
    )
    parser.add_argument(
        "--lu", type=float, default=0.5, help="Regularization factor, default 0.5."
    )
    parser.add_argument(
        "--resume", type=str, help="Path to the checkpoint (default: none)"
    )
    parser.add_argument(
        "--new", action="store_true", help="Train the model from the begining."
    )
    parser.add_argument(
        "--scheduler-step",
        type=int,
...
def train_model(
    model: AFB_URR,
    dataloader: data.DataLoader[Any],
    ...
    for _iter_idx, sample in enumerate(progress_bar):
        frames, masks, obj_n, _info = sample
...
if __name__ == "__main__":
    ...
    if args.log:
        ...
        myutils.save_scripts(
            log_dir, scripts_to_save=glob("dataset/*.py", recursive=True)
        )
        myutils.save_scripts(
            log_dir, scripts_to_save=glob("model/*.py", recursive=True)
        )
        myutils.save_scripts(
            log_dir, scripts_to_save=glob("myutils/*.py", recursive=True)
        )
